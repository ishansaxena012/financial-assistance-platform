...
// Maps persona slug → emoji fallback
...
export default function HomeScreen() {
  ...
  // Rotate tip daily (stable across renders in same session)
  ...
  const handleStartVoice = () => {
    useAppStore.setState({ isVoiceMode: true });
    setStep("language");
  };
    ...
    <>
      <style>{`
        ...
        .hs-wrap {
          ...
          position: relative;
          overflow-x: hidden;
          /* enough room below last section — no fixed sheet to worry about */
          padding-bottom: 40px;
        }

        /* ── stagger animation: one parent, children get delay ── */
        .hs-stagger > * {
          opacity: 0;
          animation: hsSlideUp 0.5s cubic-bezier(0.22, 1, 0.36, 1) forwards;
        }
        .hs-stagger > *:nth-child(1) { animation-delay: 0.05s; }
        .hs-stagger > *:nth-child(2) { animation-delay: 0.15s; }
        .hs-stagger > *:nth-child(3) { animation-delay: 0.25s; }
        .hs-stagger > *:nth-child(4) { animation-delay: 0.35s; }
        .hs-stagger > *:nth-child(5) { animation-delay: 0.45s; }
        @keyframes hsSlideUp {
        ...
        .hs-action-card {
          ...
          /* no box-shadow — avoids scale rendering artifact */
        }
        ...
        .hs-sess:hover { border-color: #D4A853; transform: translateX(2px); }

        /* ── profile overlay ── */
        /* Uses position:absolute (relative to .hs-wrap) — NOT fixed —
           so overflow-x:hidden on the wrapper doesn't clip it,
           and it works inside nested scroll contexts */
        .hs-profile-overlay {
          position: absolute;
          inset: 0;
          z-index: 100;
          /* pointer passthrough on backdrop handled by inner element */
        }
        .hs-profile-backdrop {
          position: absolute;
          inset: 0;
      ...
      <div className="hs-wrap">
        ...
        {/* ── Profile overlay (absolute, not fixed) ── */}
        {isProfileOpen && (
          <div className="hs-profile-overlay">
            ...
            <div className="hs-profile-card">
              ...
              {/* Session History (App Drawer) */}
              <div style={{ flex: 1, overflowY: "auto", padding: "16px", background: "white" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
                  <History size={12} color="#9B5A3C" />
                  <span style={{ fontSize: 11, fontWeight: 600, color: "#9B5A3C", textTransform: "uppercase", letterSpacing: "0.1em" }}>
                    Recent Sessions
                  </span>
                </div>

                {isLoading && (
                  <p style={{ fontSize: 12, color: "#9B5A3C", fontStyle: "italic" }}>Loading history…</p>
                )}
                ...
                {!isLoading && sessions.map((sess) => (
                  <div key={sess.id} className="hs-sess" style={{ marginBottom: 6, padding: "8px 10px", borderRadius: 10 }} onClick={() => { setIsProfileOpen(false); resumeSession(sess.id); }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: 10, color: "#9B5A3C", margin: "2px 0 0", whiteSpace: "nowrap" }}>
                        {new Date(sess.created_at).toLocaleDateString("en-IN", { month: "short", day: "numeric" })}
                      </p>
                    </div>
        ...
        {/* ── All stagger children live inside ONE parent ── */}
        <div className="hs-stagger">
          ...
          <div style={{ padding: "16px 16px 0" }}>
            <div className="hs-wisdom">
              {/* Decorative rings — pure CSS, no image dependency */}
              ...
              {/* Tip chip — no image, so no broken-image gap */}
          ...
          {/* 5 — FD Calculator shortcut */}
          <div style={{ margin: "0 16px" }}>
            <div
              onClick={() => setStep("fd-calc")}
              style={{ background: "white", border: "1px solid rgba(212,168,83,0.45)", borderRadius: 18, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", transition: "transform 0.2s" }}
              onMouseEnter={(e) => (e.currentTarget.style.transform = "translateY(-2px)")}
              onMouseLeave={(e) => (e.currentTarget.style.transform = "")}
              onMouseDown={(e) => (e.currentTarget.style.transform = "scale(0.98)")}
              onMouseUp={(e) => (e.currentTarget.style.transform = "")}
            >
              <div style={{ width: 44, height: 44, borderRadius: 12, background: "#FDF6E7", border: "1px solid rgba(212,168,83,0.35)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: 11, color: "#9B5A3C", margin: "2px 0 0" }}>Calculate your maturity amount</p>
              </div>
              <ArrowRight size={16} color="rgba(212,168,83,0.7)" />
            </div>
          </div>
